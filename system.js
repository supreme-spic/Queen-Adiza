...
const tylorkid6 = fs.readFileSync("./Media/Images/jin6.jpg");
const tylorkid7 = fs.readFileSync("./Media/Images/jin7.jpg");
const tylorkid8 = fs.readFileSync("./Media/Images/jin8.jpg");
const tylorkid9 = fs.readFileSync("./Media/Images/jin9.jpg");
const tylorkid10 = fs.readFileSync("./Media/Images/jin10.jpg");
const tylorkid11 = fs.readFileSync("./Media/Images/jin11.jpg");
const tylorkid12 = fs.readFileSync("./Media/Images/jin12.jpg");
const tylorkid13 = fs.readFileSync("./Media/Images/jin13.jpg");
const tylorkid14 = fs.readFileSync("./Media/Images/jin14.jpg");
  ...
  try {
    ...
    try {
      if (_0x1f0527.endsWith("@g.us")) {
        ...
        if (!("antispam1" in _0x4e02f1)) {
          _0x4e02f1.antispam1 = false;
        }
...
if (
  ...
  db.data.chats[_0x50668e.chat]?.antispam1
) {
  const chatId   = _0x50668e.chat;
  const senderId = _0x50668e.sender;
  const now      = _0x50668e.messageTimestamp * 1000;
  const isAdmin  = (await getGroupAdmins(chatId)).includes(senderId);

  if (!isAdmin) {
    const user = (global.spamCounts[senderId] =
      global.spamCounts[senderId] || {
        count: 0,
        lastTimestamp: 0,
        blockedUntil: 0,
        deleteCount: 0,
    ...
    // 1) If still in block window → delete 8th/9th or kick on 10th
...
┃ removed from the group due to repeated
┃ spamming.
┗━━━━━━━━━━━━━━━━━━━━━━┛`,
    ...
    // 2) Not in block window → rolling‐window count
...
┃  ⚠️ *SPAM WARNING* ⚠️  
┣━━━━━━━━━━━━━━━━━━━━━━┫
┃ @${senderId.split("@")[0]}, you’re sending
┃ messages too fast. This is a warning.
┃
┃ 📅 ${dateStr}
┃ ⏰ ${timeStr}
┃
┃ On your 7th msg you’ll be blocked.
┗━━━━━━━━━━━━━━━━━━━━━━┛`,
        contextInfo: { mentionedJid: [senderId] }
      }, { quoted: _0x50668e });
      return;
    ...
    // 4) 7th message → start 10 min block & schedule unblock notice
    if (user.count === 7) {
      user.blockedUntil = now + 10 * 60 * 1000; // 10 minutes
      user.deleteCount  = 0;
...
┃  ⛔ *TEMP BLOCKED* ⛔  
┣━━━━━━━━━━━━━━━━━━━━━━┫
┃ @${senderId.split("@")[0]} you’ve been
┃ blocked for *10 minutes* due to spamming.
...
┗━━━━━━━━━━━━━━━━━━━━━━┛`,
      ...
      // schedule unblock notice at blockedUntil
      user.unblockTimer = setTimeout(async () => {
        const liftT = user.blockedUntil;
        const liftDate = new Date(liftT);
        const unblockDateStr = liftDate.toLocaleDateString("en-US", {
          weekday: "long", year: "numeric", month: "long", day: "numeric",
          timeZone: "Africa/Accra"
        });
        const unblockTimeStr = liftDate.toLocaleTimeString("en-US", {
          hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true,
          timeZone: "Africa/Accra"
        });
...
┗━━━━━━━━━━━━━━━━━━━━━━┛`
        ...
        await _0x1548f3.sendMessage(senderId, {
          text: `┏━━━━━━━━━━━━━━━━━━━━━━┓
┃  ✅ *BLOCK LIFTED* ✅  
┣━━━━━━━━━━━━━━━━━━━━━━┫
...
if (!("antilinkgckick" in _0x4e02f1)) {
      ...
      if (!("adizabot" in _0x2f3da5)) {
        _0x2f3da5.adizabot = false;
      }
      if (!("natashabot" in _0x2f3da5)) {
        _0x2f3da5.natashabot = false;
      }
...
if (_0x1f0527.endsWith("@g.us") && db.data.chats[_0x50668e.chat]?.antiimage) {
  if (_0x50668e.message?.imageMessage && !_0x419a1c && !_0x22e08b && _0x26d2c7) {
    ...
    if (_0x50668e.quoted && _0x50668e.quoted.viewOnce && (_0x50668e.message?.conversation || _0x50668e.message?.extendedTextMessage) && _0x22e08b && ["😀", "😃", "😄", "😁", "😆", "😅", "😂", "🤣", "🥲", "🥹", "😊", "😇", "🙂", "🙃", "😉", "😌", "😍", "🥰", "😘", "😗", "😙", "😚", "😋", "😛", "😝", "😜", "🤪", "🤨", "🧐", "🤓", "😎", "🥸", "🤩", "🥳", "😏", "😒", "😞", "😔", "😟", "😕", "🙁", "☹️", "😣", "😖", "😫", "😩", "🥺", "😢", "😭", "😤", "😠", "😡", "🤬", "🤯", "😳", "🥵", "🥶", "😱", "😨", "😰", "😥", "😓", "🫣", "🤗", "🫡", "🤔", "🫢", "🤭", "🤫", "🤥", "😶", "😶‍🌫️", "😐", "😑", "😬", "🫠", "🙄", "😯", "😦", "😧", "😮", "😲", "🥱", "😴", "🤤", "😪", "😵", "😵‍💫", "🫥", "🤐", "🥴", "🤢", "🤮", "🤧", "😷", "🤒", "🤕", "🤑", "🤠", "😈", "👿", "👹", "👺", "🤡", "💩", "👻", "💀", "☠️", "👽", "👾", "🤖", "🎃", "😺", "😸", "😹", "😻", "😼", "😽", "🙀", "😿", "😾", "👋", "🤚", "🖐️", "✋", "🖖", "👌", "🤌", "🤏", "✌️", "🤞", "🫰", "🤟", "🤘", "🤙", "👈", "👉", "👆", "🖕", "👇", "☝️", "👍", "👎", "✊", "👊", "🤛", "🤜", "👏", "🫶", "🙌", "👐", "🤲", "🤝", "🙏", "✍️", "💅", "🤳", "💪", "🦾", "🦿", "🦵", "🦶", "👂", "🦻", "👃", "🧠", "🫀", "🫁", "🦷", "🦴", "👀", "👁️", "👅", "👄", "🫦", "👶", "🧒", "👦", "👧", "🧑", "👱", "👨", "🧔", "🧔‍♂️", "🧔‍♀️", "👨‍🦰", "👨‍🦱", "👨‍🦳", "👨‍🦲", "👩", "👩‍🦰", "👩‍🦱", "👩‍🦳", "👩‍🦲", "🧓", "👴", "👵", "🙍", "🙍‍♂️", "🙍‍♀️", "🙎", "🙎‍♂️", "🙎‍♀️", "🙅", "🙅‍♂️", "🙅‍♀️", "🙆", "🙆‍♂️", "🙆‍♀️", "💁", "💁‍♂️", "💁‍♀️", "🙋", "🙋‍♂️", "🙋‍♀️", "🧏", "🧏‍♂️", "🧏‍♀️", "🙇", "🙇‍♂️", "🙇‍♀️", "🤦", "🤦‍♂️", "🤦‍♀️", "🤷", "🤷‍♂️", "🤷‍♀️", "👮", "👮‍♂️", "👮‍♀️", "🕵️", "🕵️‍♂️", "🕵️‍♀️", "💂", "💂‍♂️", "💂‍♀️", "🥷", "👷", "👷‍♂️", "👷‍♀️", "🫅", "🤴", "👸", "👳", "👳‍♂️", "👳‍♀️", "👲", "🧕", "🤵", "🤵‍♂️", "🤵‍♀️", "👰", "👰‍♂️", "👰‍♀️", "🫃", "🫄", "🤰", "🤱", "👩‍🍼", "👨‍🍼", "🧑‍🍼", "👼", "🎅", "🧑‍🎄", "🦸", "🦸‍♂️", "🦸‍♀️", "🦹", "🦹‍♂️", "🦹‍♀️", "🧙", "🧙‍♂️", "🧙‍♀️", "🧚", "🧚‍♂️", "🧚‍♀️", "🧛", "🧛‍♂️", "🧛‍♀️", "🧜", "🧜‍♂️", "🧜‍♀️", "🧝", "🧝‍♂️", "🧝‍♀️", "🧞", "🧞‍♂️", "🧞‍♀️", "🧟", "🧟‍♂️", "🧟‍♀️", "🧌", "💆", "💆‍♂️", "💆‍♀️", "💇", "💇‍♂️", "💇‍♀️", "🚶", "🚶‍♂️", "🚶‍♀️", "🧍", "🧍‍♂️", "🧍‍♀️", "🧎", "🧎‍♂️", "🧎‍♀️", "🏃", "🏃‍♂️", "🏃‍♀️", "💃", "🕺", "🕴️", "👯", "👯‍♂️", "👯‍♀️", "🧖", "🧖‍♂️", "🧖‍♀️", "🧗", "🧗‍♂️", "🧗‍♀️", "🤺", "🏇", "⛷️", "🏂", "🏌️", "🏌️‍♂️", "🏌️‍♀️", "🏄", "🏄‍♂️", "🏄‍♀️", "🚣", "🚣‍♂️", "🚣‍♀️", "🏊", "🏊‍♂️", "🏊‍♀️", "⛹️", "⛹️‍♂️", "⛹️‍♀️", "🏋️", "🏋️‍♂️", "🏋️‍♀️", "🚴", "🚴‍♂️", "🚴‍♀️", "🚵", "🚵‍♂️", "🚵‍♀️", "🤸", "🤸‍♂️", "🤸‍♀️", "🤼", "🤼‍♂️", "🤼‍♀️", "🤽", "🤽‍♂️", "🤽‍♀️", "🤾", "🤾‍♂️", "🤾‍♀️", "🤹", "🤹‍♂️", "🤹‍♀️", "🧘", "🧘‍♂️", "🧘‍♀️", "🛀", "🛌", "🧑‍🤝‍🧑", "👭", "👫", "👬", "💏", "👩‍❤️‍💋‍👨", "👨‍❤️‍💋‍👨", "👩‍❤️‍💋‍👩", "💑", "👩‍❤️‍👨", "👨‍❤️‍👨", "👩‍❤️‍👩", "👪", "🧑‍👦", "🧑‍👧", "🧑‍👧‍👦", "🧑‍👦‍👦", "🧑‍👧‍👧", "👨‍👦", "👨‍👧", "👨‍👧‍👦", "👨‍👦‍👦", "👨‍👧‍👧", "👩‍👦", "👩‍👧", "👩‍👧‍👦", "👩‍👦‍👦", "👩‍👧‍👧", "🗣️", "👤", "👥", "🫂", "👣", "🧳", "🌂", "☂️", "🎃", "🧵", "🧶", "👓", "🕶️", "🥽", "🥼", "🦺", "👔", "👕", "👖", "🧣", "🧤", "🧥", "🧦", "👗", "👘", "🥻", "🩱", "🩲", "🩳", "👙", "👚", "👛", "👜", "👝", "🎒", "🩴", "👞", "👟", "🥾", "🥿", "👠", "👡", "👢", "👑", "👒", "🎩", "🎓", "🧢", "🪖", "⛑️", "📿", "💄", "💍", "💎", "🐵", "🐒", "🦍", "🦧", "🐶", "🐕", "🦮", "🐕‍🦺", "🐩", "🐺", "🦊", "🦝", "🐱"].some(_0xe8b1e7 => _0x50668e.body.startsWith(_0xe8b1e7))) {
      (async () => {
        try {
...
if (
  db.data.settings.chatbot === true &&
  (_0x50668e.isGroup || !_0x50668e.isGroup) && // Allow in both groups and private chats
  (_0x50668e.message?.extendedTextMessage?.text || _0x50668e.message?.conversation)
) {
  const _0xraw = _0x50668e.message?.extendedTextMessage?.text || _0x50668e.message?.conversation || "";
  const _0xisChatbot = _0xraw.startsWith("?");

  // Only allow chatbot for three specific chatbots (e.g., 'openai', 'meta', 'bk9')
  if (_0xisChatbot) {
    try {
      const senderId = _0x50668e.sender;
      const rawText = _0x50668e.message?.extendedTextMessage?.text || _0x50668e.message?.conversation || "";

      // Only respond if the message starts with "?"
      if (!rawText.startsWith("?")) return;

      const prompt = rawText.slice(1).trim(); // remove "?" prefix
      if (!prompt) return;

      if (prompt.toLowerCase() === "reset") {
        chatHistory[senderId] = [];
        return await _0x1548f3.sendMessage(_0x50668e.chat, { text: "Chat history reset!" }, { quoted: _0x50668e });
      }

      if (!chatHistory[senderId]) chatHistory[senderId] = [];
      chatHistory[senderId].push({ role: "user", content: prompt });

      if (chatHistory[senderId].length > 6) {
        chatHistory[senderId] = chatHistory[senderId].slice(-6);
      }

      await _0x1548f3.sendPresenceUpdate("composing", _0x50668e.chat);
      await _0x1548f3.sendMessage(_0x50668e.chat, { react: { text: "⏱️", key: _0x50668e.key } });

      const conversation = chatHistory[senderId]
      ...
      const urls = [
        ...
        `https://bk9.fun/ai/jeeves-chat?q=${encodeURIComponent(prompt)}`
      ];
      ...
      for (let i = 0; i < urls.length; i++) {
        try {
          ...
          const data = res.data;
          replyText = data.data || data.BK9 || data.response || data.result;
          if (replyText) break;
        } catch (e) {
          console.log("API failed:", urls[i]);
        }
      }

      if (replyText) {
        chatHistory[senderId].push({ role: "assistant", content: replyText });
        await _0x1548f3.sendMessage(_0x50668e.chat, { text: replyText }, { quoted: _0x50668e });
        await _0x1548f3.sendMessage(_0x50668e.chat, { react: { text: "💬", key: _0x50668e.key } });
      } else {
        await _0x1548f3.sendMessage(_0x50668e.chat, { text: "❌ No valid response received." }, { quoted: _0x50668e });
      }

    } catch (_0x55d0ec) {
      console.error("Chatbot Error:", _0x55d0ec);
      await _0x1548f3.sendMessage(_0x50668e.chat, { text: "⚠️ Error while processing request." }, { quoted: _0x50668e });
    }
  }
}
...
if (
  db.data.settings.chatbot === true &&
  !_0x50668e.isGroup &&
  !_0x1a38e3 && // Make sure this represents `isCmd === false`
  (_0x50668e.message?.extendedTextMessage?.text || _0x50668e.message?.conversation)
) {
  try {
    const senderId = _0x50668e.sender;
    const prompt = _0x50668e.message?.extendedTextMessage?.text || _0x50668e.message?.conversation;
    ...
    if (!chatHistory[senderId]) chatHistory[senderId] = [];
    chatHistory[senderId].push({ role: "user", content: prompt });
    if (chatHistory[senderId].length > 6) chatHistory[senderId] = chatHistory[senderId].slice(-6);
    ...
    console.error("Natashabot Error:", err);
    await _0x1548f3.sendMessage(_0x50668e.chat, { text: "⚠️ Error while processing request." }, { quoted: _0x50668e });
  }
}
    function _0x5956de() {
      ...
      case "menu":
        ...
        const _0x4ba61c = (_0x292ced, _0x7fd380, _0x201b3c, _0xa4c7c7, _0x33514f, _0xc3ee53, _0x1746ba) => {
          ...
          const _0x277b6d = (_0x3463fb, _0x43f39d, _0x870af = 10) => {
            ...
            let _0x479819 = "█".repeat(_0x459442) + "░".repeat(_0x870af - _0x459442);
          ...
          let _0x310126 = "╭༺◈👸🌹𝗔𝗗𝗜𝗭𝗔𝗧𝗨🌹👸\n";
          _0x310126 += "│🇬🇭 *ᴏᴡɴᴇʀ* : " + _0x7fd380 + "\n";
          _0x310126 += "│⚡ *ᴘʀᴇғɪx* : [ " + _0x201b3c + " ]\n";
          _0x310126 += "│💻 *ʜᴏsᴛ* : " + os.platform() + "\n";
          _0x310126 += "│🧩 *ᴘʟᴜɢɪɴs* : " + _0x25e6ef + "\n";
          _0x310126 += "│🚀 *ᴍᴏᴅᴇ* : " + _0xa4c7c7 + "\n";
          _0x310126 += "│🛠️ *ᴠᴇʀsɪᴏɴ* : " + _0x33514f + "\n";
          _0x310126 += "│⚡ *sᴘᴇᴇᴅ* : " + _0xc3ee53.toFixed(4) + " ms\n";
          _0x310126 += "│📈 *ᴜsᴀɢᴇ* : " + _0x1ecb54(_0x3a3793) + " of " + _0x1ecb54(_0x4497bb) + "\n";
          _0x310126 += "│🧠 *ʀᴀᴍ* : " + _0x277b6d(_0x18d668, _0x4497bb) + "\n";
          _0x310126 += "╰───━━━༺◈༻━━━───╯\n" + _0x1746ba + "\n";
        ...
        await _0x50668e.reply("🚀𝙇𝙤𝙖𝙙𝙞𝙣𝙜....𝘼𝙙𝙞𝙯𝙖-𝘽𝙤𝙩🌹");
