  ...
  operate: async ({ Cypher, m, reply }) => {
    ...
    try {
      ...
      const { data: zipData } = await axios.get(
        "https://github.com/Matri199/Queen-Adiza/archive/refs/heads/main.zip",
      ...
      // Extract ZIP
      const extractPath = path.join(__dirname, "update-tmp");
      ...
      const newCodePath = path.join(extractPath, "Queen-Adiza-main");
      ...
      // Copy files to root project
      copyFolderSync(newCodePath, process.cwd());

      await reply(steps[3]);

      // Save the commit hash in package.json
      const { data: commitData } = await axios.get(
        "https://api.github.com/repos/Matri199/Queen-Adiza/commits/main"
      );
      ...
      process.exit(0); // Let the panel auto-restart the bot
    } catch (e) {
